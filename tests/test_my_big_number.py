...
def test_show_steps_prints_operation_history(capsys):
    result = main("99", "1", show_steps=True)
    printed_lines = capsys.readouterr().out.strip().splitlines()

    assert result == "100"
    assert "Step 1: 9 + 1 + carry 0 = 10; write 0, next carry 1" in printed_lines
    assert printed_lines[-1] == "100"
